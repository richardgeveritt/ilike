...
void SequentialSMCWorker::specific_simulate(Particles* next_particles)
{
  ...
  {
    Particle new_p = this->the_smc->particle_simulator->simulate(local_rng,
                                                                 this->the_smc->factors);
    
    //std::cout<<new_p.factor_variables->get_particle()->parameters<<std::endl;
                                 ...
                                 Particles &current_particles)
{
  ...
  for (size_t i = 0; i < this->get_number_of_particles(); ++i)
  {
    //std::cout<<current_particles[i]->back().parameters<<std::endl;
