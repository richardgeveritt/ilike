...
inline arma::mat vector_of_parameters_to_mat(const std::vector<Parameters> &param_vec)
{
  if (param_vec.size()==0)
    return arma::mat(0,0);
  
  arma::colvec first_vector = param_vec.begin()->get_vector();
  
  arma::mat output(param_vec.size(),first_vector.n_rows);
  output.row(0) = first_vector;
  for (size_t i=1; i<param_vec.size(); ++i)
  {
    output.row(i) = param_vec[i].get_vector();
  }
  return output;
}
                                             ...
                                             const std::vector<Parameters> &param_vec)
{
  ...
  arma::colvec first_vector = param_vec.begin()->get_vector(variable);
  
  arma::mat output(param_vec.size(),first_vector.n_rows);
  output.row(0) = first_vector;
  for (size_t i=1; i<param_vec.size(); ++i)
  {
    output.row(i) = param_vec[i].get_vector(variable);
  }
  return output;
}
